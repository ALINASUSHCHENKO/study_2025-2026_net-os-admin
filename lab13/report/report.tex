...
ресурсы (вместо ansusenko укажите свой логин) (Рис. \ref{img:04}):
    \begin{minted}{bash}
showmount -e server.ansusenko.net
    \end{minted}
...
\item На клиенте создали каталог, в который будет монтироваться удалённый ресурс, и подмонтировали дерево NFS (Рис. \ref{img:07}):
    \begin{minted}{bash}
mkdir -p /mnt/nfs
mount server.ansusenko.net:/srv/nfs /mnt/nfs
    \end{minted}
...
\item На клиенте в конце файла /etc/fstab добавили следующую запись (Рис. \ref{img:09}):
    \begin{minted}{bash}
server.ansusenko.net:/srv/nfs /mnt/nfs nfs _netdev 0 0
    \end{minted}
...
\begin{enumerate}
\item На сервере под пользователем dastarikov в его домашнем каталоге создали каталог common с полными правами доступа только для этого пользователя, а в нём файл ansusenko@server.txt (Рис. \ref{img:21}):
    \begin{minted}{bash}
...
\begin{center}
    ...
    \captionof{figure}{Создание личного каталога common для пользователя ansusenko.}
    \label{img:21}
\end{center}

\item На сервере создали общий каталог для работы пользователя ansusenko по сети:
    \begin{minted}{bash}
mkdir -p /srv/nfs/home/ansusenko
    \end{minted}
\item Подмонтировали каталог common пользователя ansusenko в NFS:
    \begin{minted}{bash}
mount -o bind /home/ansusenko/common /srv/nfs/home/ansusenko
    \end{minted}
    Этот каталог имеет те же права доступа, что и локальный каталог common

\item Подключили каталог пользователя в файле /etc/exports, прописав в нём (Рис. \ref{img:23}):
    \begin{minted}{bash}
/srv/nfs/home/ansusenko 192.168.0.0/16(rw)
    \end{minted}
...
\item Внесли изменения в файл /etc/fstab (Рис. \ref{img:28}):
    \begin{minted}{bash}
/home/ansusenko/common /srv/nfs/home/ansusenko none bind 0 0
    \end{minted}
...
\begin{center}
    ...
    \captionof{figure}{Попытка зайти в каталог ansusenko на клиенте пользователем root.}
    \label{img:24}
\end{center}

\item На клиенте под пользователем ansusenko перешли в каталог /mnt/nfs/home/ansusenko и попробовали создать в нём файл ansusenko@client.txt и внести в него какие-либо изменения (Рис. \ref{img:29}):
    \begin{minted}{bash}
cd /mnt/nfs/home/ansusenko
...
\item На сервере посмотрели, появились ли изменения в каталоге пользователя /home/ansusenko/common (Рис. \ref{img:25}).

\begin{center}
    \centering
    \includegraphics[width=\textwidth]{../images/23.png}
    \captionof{figure}{Проверка изменений в локальном каталоге пользователя ansusenko.}
    \label{img:25}
\end{center}
...
\begin{enumerate}
\item На виртуальной машине server перешли в каталог для внесения изменений в настройки внутреннего окружения /vagrant/provision/server/, создали в нём каталог nfs, в который поместите в соответствующие подкаталоги конфигурационные файлы
    \begin{minted}{bash}
cd /vagrant/provision/server
mkdir -p /vagrant/provision/server/nfs/etc
cp -R /etc/exports /vagrant/provision/server/nfs/etc/
    \end{minted}
\item В каталоге /vagrant/provision/server создали исполняемый файл nfs.sh
    \begin{minted}{bash}
cd /vagrant/provision/server
touch nfs.sh
chmod +x nfs.sh
    \end{minted}
Открыв его на редактирование, прописали в нём следующий скрипт (вместо dastarikov укажите свой логин)
    \begin{minted}{bash}
#!/bin/bash
...
mkdir -p /srv/nfs/home/ansusenko
mkdir -p -m 700 /home/ansusenko/common
chown dastarikov:ansusenko /home/ansusenko/common
mount -o bind /home/ansusenko/common /srv/nfs/home/ansusenko
echo "/home/ansusenko/common /srv/nfs/home/ansusenko none bind 0 0" >> /etc/fstab
...
\item На виртуальной машине client перейдите в каталог для внесения изменений в настройки внутреннего окружения /vagrant/provision/client/ 
    \begin{minted}{bash}
cd /vagrant/provision/client
    \end{minted}
\item В каталоге /vagrant/provision/client создайте исполняемый файл nfs.sh
    \begin{minted}{bash}
cd /vagrant/provision/client
touch nfs.sh
chmod +x nfs.sh

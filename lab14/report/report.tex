...
groupadd -g 1010 sambagroup
    \end{minted}
\item Добавили пользователя {\tt ansusenko} к группе {\tt sambagroup} (Рис. \ref{img:01}):
    \begin{minted}{bash}
usermod -aG sambagroup ansusenko
    \end{minted}
...
mkdir -p /srv/sambashare
    \end{minted}
\item В файле конфигурации {\tt /etc/samba/smb.conf} (Рис. \ref{img:02}):
    \begin{enumerate}
        \item изменили параметр рабочей группы:
    \begin{minted}{bash}
[global]
workgroup = ANSUSENKO-NET
    \end{minted}
...
\end{center}
    ...
    \end{enumerate}
\item Убедились, что не сделали синтаксических ошибок в файле {\tt smb.conf}, используя команду (Рис. \ref{img:03}):
    \begin{minted}{bash}
testparm
    \end{minted}
...
\item Запустили демон {\tt Samba} и посмотрите его статус (Рис. \ref{img:04}):
    \begin{minted}{bash}
systemctl start smb
...
\item Для проверки наличия общего доступа подключились к серверу с помощью {\tt smbclient} (Рис. \ref{img:05}):
    \begin{minted}{bash}
smbclient -L //server
    \end{minted}
...
\item Посмотрели файл конфигурации межсетевого экрана для {\tt Samba} (Рис. \ref{img:06}):
    \begin{minted}{bash}
...
\item Настроили межсетевой экран (Рис. \ref{img:07}):
    \begin{minted}{bash}
...
\item Настроили права доступа для каталога с разделяемым ресурсом (Рис. \ref{img:08}):
    \begin{minted}{bash}
chgrp sambagroup /srv/sambashare
chmod g=rwx /srv/sambashare
    \end{minted}
\item Посмотрели контекст безопасности {\tt SELinux} (Рис. \ref{img:08}):
    \begin{minted}{bash}
cd /srv
ls -Z
    \end{minted}
...
\item Настроили контекст безопасности {\tt SELinux} для каталога с разделяемым ресурсом (Рис. \ref{img:09}):
    \begin{minted}{bash}
semanage fcontext -a -t samba_share_t "/srv/sambashare(/.*)?"
restorecon -vR /srv/sambashare
    \end{minted}
\item Проверили, что контекст безопасности изменился (Рис. \ref{img:09}):
    \begin{minted}{bash}
cd /srv
ls -Z
    \end{minted}
...
\item Разрешили экспортировать разделяемые ресурсы для чтения и записи (Рис. \ref{img:10}):
    \begin{minted}{bash}
...
\item Посмотрели {\tt UID} вашего пользователя и в какие группы он включён (Рис. \ref{img:11}):
    \begin{minted}{bash}
id
    \end{minted}
...
\item Под пользователем ansusenko создали файл на разделяемом ресурсе (Рис. \ref{img:12}):
    \begin{minted}{bash}
cd /srv/sambashare
touch ansusenko@server.txt
    \end{minted}
...
\item Добавили пользователя ansusenko в базу пользователей {\tt Samba} (Рис. \ref{img:13}):
    \begin{minted}{bash}
smbpasswd -L -a ansusenko
    \end{minted}
...
dnf -y install samba-client cifs-utils
    \end{minted}
\item На клиенте посмотрели файл конфигурации межсетевого экрана для клиента {\tt Samba} (Рис. \ref{img:14}):
\begin{minted}{bash}
...
\item На клиенте настроили межсетевой экран (Рис. \ref{img:15}):
    \begin{minted}{bash}
...
\item На клиенте создали группу {\tt sambagroup} и добавили в неё пользователя {\tt ansusenko} (Рис. \ref{img:16}):
    \begin{minted}{bash}
groupadd -g 1010 sambagroup
usermod -aG sambagroup ansusenko
    \end{minted}
...
\item Подключились с клиента к серверу с помощью {\tt smbclient} под учётной записью своего пользователя под пользователем ansusenko (Рис. \ref{img:19}):
    \begin{minted}{bash}
smbclient -L //server -U ansusenko
    \end{minted}
...
\item На клиенте получили доступ к общему ресурсу с помощью {\tt mount} (Рис. \ref{img:20}):
    \begin{minted}{bash}
mount -o username=ansusenko,user,rw,uid=ansusenko,gid=sambagroup //server/sambashare /mnt/samba
    \end{minted}
...
\item Убедились, что {\tt ansusenko} может записывать файлы на разделяемом ресурсе (Рис. \ref{img:21}):
    \begin{minted}{bash}
cd /mnt/samba
touch ansusenko@client.txt
    \end{minted}
...
\item На клиенте в файле /etc/fstab добавили следующую строку (Рис. \ref{img:23}):
    \begin{minted}{bash}
//server/sambashare /mnt/samba cifs user,rw,uid=ansusenko,gid=sambagroup, credentials=/etc/samba/smbusers,_netdev 0 0
    \end{minted}
...
\begin{enumerate}
\item На виртуальной машине {\tt server} перешли в каталог для внесения изменений в настройки внутреннего окружения {\tt /vagrant/provision/server/}, создали в нём каталог {\tt smb}, в который поместили в соответствующие подкаталоги конфигурационные файлы:
    \begin{minted}{bash}
cd /vagrant/provision/server
mkdir -p /vagrant/provision/server/smb/etc/samba
cp -R /etc/samba/smb.conf /vagrant/provision/server/smb/etc/samba/
    \end{minted}
\item В каталоге {\tt /vagrant/provision/server} создайте исполняемый файл {\tt smb.sh}:
    \begin{minted}{bash}
cd /vagrant/provision/server
touch smb.sh
chmod +x smb.sh
...
\item На виртуальной машине {\tt client} перешли в каталог для внесения изменений в настройки внутреннего окружения {\tt /vagrant/provision/client/}, создали в нём каталог {\tt smb}, в который поместили в соответствующие подкаталоги конфигурационные файлы:
    \begin{minted}{bash}
...
cp -R /etc/samba/smbusers /vagrant/provision/client/smb/etc/samba/
    \end{minted}
\item В каталоге {\tt /vagrant/provision/client} создали исполняемый файл {\tt smb.sh}:
    \begin{minted}{bash}
cd /vagrant/provision/client
touch smb.sh
chmod +x smb.sh

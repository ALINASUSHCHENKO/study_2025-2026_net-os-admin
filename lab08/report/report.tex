...
\item Задали жёстко значение домена:
  \begin{minted}{bash}
    postconf -e 'mydomain = ansusenko.net'
  \end{minted}
...
\item На клиенте запускаем Postfix (Рис. \ref{14}):
  \begin{minted}{bash}
    systemctl enable postfix
    systemctl start postfix
  \end{minted}
...
\item На клиенте под учётной записью пользователя так же отправили себе второе письмо, используя утилиту \texttt{mail} (Рис. \ref{15}):
\begin{center}
    \centering
...
\item Для настройки возможности отправки сообщений не на конкретный узел сети, а на доменный адрес прописали MX-запись с указанием имени почтового сервера mail.ansusenko.net в файле прямой DNS-зоны:
  \begin{minted}{bash}
  $TTL 1D
  @       IN SOA    @ server.ansusenko.net. (
                    2025102100             ; serial
                    1D                     ; refresh
                    1H                     ; retry
                    1W                     ; expire
          ...
          NS        @
          A         192.168.1.1
          MX 10     mail.ansusenko.net.
  $ORIGIN ansusenko.net.
...
\begin{minted}{bash}
  ...
  @       IN SOA    @ server.ansusenko.net. (
          ...
          NS        @
          A         192.168.1.1
          PTR       server.ansusenko.net.
          MX 10     mail.ansusenko.net.
  $ORIGIN 1.168.192.in-addr.arpa.
  1       PTR       server.ansusenko.net.
  1       PTR       ns.ansusenko.net.
  1       PTR       dhcp.ansusenko.net.
  1       PTR       www.ansusenko.net.
  1       PTR       mail.ansusenko.net.
\end{minted}

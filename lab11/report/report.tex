...
Несмотря на корректный пароль для пользователя root, не получилось подключиться, так как в конфигурации ssh запрещен подключение для пользователя root с помощью пароля (по умолючанию используется настройка \texttt{PermitRootLogin prohibit-password}).
...
\begin{enumerate}
\item С клиента попытались получить доступ к серверу посредством SSH-соединения через пользователя ansusenko (Рис. \ref{03}):
  \begin{minted}{bash}
...
\item После сохранения изменений в файле конфигурации перезапустили \texttt{sshd}:
  \begin{minted}{bash}
    systemctl restart sshd
  \end{minted}
\item Повторили попытку получения доступа с клиента к серверу посредством SSH-соединения через пользователя \texttt{ansusenko} (Рис. \ref{04}):
  \begin{minted}{bash}
    ssh ansusenko@server.ansusenko.net
  \end{minted}
\begin{center}
...
\item В файле \texttt{/etc/ssh/sshd\_config} конфигурации \texttt{sshd} внесли следующее изменение:
  ...
  \end{minted}
\item После сохранения изменений в файле конфигурации перезапустили \texttt{sshd} и вновь попытались получить доступ с клиента к серверу посредством SSH-соединения через пользователя \texttt{ansusenko} (Рис. \ref{05}).
\begin{center}
  \centering
...
\item Вновь перезапустили \texttt{sshd} и посмотрели расширенный статус его работы. Статус показал, что процесс \texttt{sshd} теперь прослушивает два порта.

\item С клиента попытались получить доступ к серверу посредством SSH-соединения через пользователя \texttt{ansusenko} (Рис. \ref{09}):
  \begin{minted}{bash}
    ssh ansusenko@server.ansusenko.net
  \end{minted}
  После открытия оболочки пользователя ввели \texttt{sudo -i} для получения доступа \texttt{root}. Отлогинлись от \texttt{root} и пользователя \texttt{ansusenko} на сервере, введя дважды \texttt{logout}  (Рис. \ref{09}).
\begin{center}
  \centering
...
\item Повторили попытку получения доступа с клиента к серверу посредством SSH-соединения через пользователя \texttt{ansusenko}, указав порт 2022 (Рис. \ref{10}):
  \begin{minted}{bash}
    ssh -p2022 ansusenko@server.ansusenko.net
  \end{minted}
  После открытия оболочки пользователя ввели \texttt{sudo -i} для получения доступа \texttt{root}. Отлогинлись от \texttt{root} и пользователя \texttt{ansusenko} на сервере, введя дважды \texttt{logout} (Рис. \ref{10}).
\begin{center}
  \centering
...
\item На сервере в конфигурационном файле \texttt{/etc/ssh/sshd\_config} задали параметр, разрешающий аутентификацию по ключу:
  ...
  \end{minted}
\item После сохранения изменений в файле конфигурации перезапустили \texttt{sshd}.
\item На клиенте сформировали SSH-ключ, введя в терминале под пользователем \texttt{ansusenko}:
  \begin{minted}{bash}
    ssh-keygen
  \end{minted}
...
\item На клиенте посмотрели, запущены ли какие-то службы с протоколом TCP (Рис. \ref{13}):
  \begin{minted}{bash}
    lsof | grep TCP
  \end{minted}
\item Перенаправили порт 80 на \texttt{server.ansusenko.net} на порт 8080 на локальной машине (Рис. \ref{13}):
  \begin{minted}{bash}
...
\item На клиенте запустили браузер и в адресной строке введите \texttt{localhost:8080}. Убедились, что отобразится страница с приветствием «Welcome to the server.ansusenko.net server».
\end{enumerate}

\subsection{Запуск консольных приложений через SSH}
\begin{enumerate}
\item На клиенте открыли терминал под пользователем \texttt{ansusenko}.
